...
| [2. Custom self-managed MCP servers](scenarios/2.%20Custom%20self-managed%20MCP%20servers/README.md) | <ul><li><b>Run your own custom MCP servers (exchange-rate and QuickChart) next to the ADK client</b></li><li><b>2-legged OAuth — ADK exchanges client credentials at Keycloak; each MCP server verifies the JWT audience and scope</b></li><li>Connect one agent to multiple self-managed MCP servers over Streamable HTTP</li></ul> |
| [3. MCP Toolbox for databases - custom MCP server](scenarios/3.%20MCP%20Toolbox%20for%20databases%20-%20custom%20MCP%20server/README.md) | <ul><li><b>Run MCP Toolbox for Databases as a easy YAML-configured MCP server wrapping around a database</b></li><li><b>3-legged OAuth — ADK opens Keycloak login; Toolbox verifies the JWT audience and per-tool scopes</b></li><li>Role-based tool access: `admin` / `user` / `trial` accounts get different scopes</li></ul> |
